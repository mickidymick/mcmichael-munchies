...
-- ============================================
-- Stock image flag: marks recipes whose hero photo came from a stock source
-- (Pixabay) so the UI can show a "replace me" badge.
-- ============================================
ALTER TABLE recipes ADD COLUMN IF NOT EXISTS is_stock_image BOOLEAN DEFAULT FALSE;

-- AI-generated image flag: marks recipes whose hero photo was generated by
-- the AI image generator (Pollinations) so the UI can show a different badge.
ALTER TABLE recipes ADD COLUMN IF NOT EXISTS is_ai_generated BOOLEAN DEFAULT FALSE;

-- ============================================
-- Recipe type: distinguishes true family recipes from personal favorites
-- found elsewhere (internet, cookbooks, etc). Family-attributed recipes keep
-- their provenance; personal favorites are tracked to the user who added them.
-- ============================================
ALTER TABLE recipes ADD COLUMN IF NOT EXISTS recipe_type TEXT NOT NULL DEFAULT 'family_recipe';
...
ALTER TABLE recipes ADD CONSTRAINT recipes_recipe_type_check
  CHECK (recipe_type IN ('family_recipe', 'personal_favorite'));
